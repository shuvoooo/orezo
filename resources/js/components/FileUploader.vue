<template>
    ...
    >
        ...
        <div v-show="UploadedFiles.length == 0" class="beforeUpload">
            <input
                type="file"
                style="z-index: 1"
                accept="image/*"
                ref="uploadInput"
                @change="previewUploadedFiles"
                multiple
            />
            ...
            <p class="mainMessage small">
                {{ uploadMsg ? uploadMsg : "Click Or Drop here" }}
            </p>
        </div>

        <div class="imgsPreview" v-show="UploadedFiles.length > 0">
            <button type="button" class="clearButton" @click="reset">
                {{ clearAll ? clearAll : "&times;" }}
            </button>
            <div class="imageHolder" v-for="(name, i) in UploadedFiles" :key="i">
                <!--                <img :src="img"/>-->
                <div>{{ name }}</div>
                <span class="delete" style="color: white" @click="deleteImg(--i)">
                    {{ deleteImg ? deleteImg : "&times;" }}
                </span>
                <div class="plus" @click="append" v-if="++i == UploadedFiles.length">+</div>
            </div>
        </div>
    </div>
</template>
