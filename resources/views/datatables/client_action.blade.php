<div class="d-flex ">
    <a href="{{route('admin.file_status.file_status',['user'=>$user['id'], 'year' => request('year') ?? date("Y")])}}"
       class="btn btn-primary btn-sm mr-1 ">File
        Status</a>

    <a href="{{route('admin.download_tax_document',['user'=>$user['id'], 'year' => request('year') ?? date("Y")])}}"
       class="btn btn-success btn-sm mr-1">Tax Docs</a>

    <a href="{{route('admin.user_tax_document',['user'=>$user['id'], 'year' => request('year') ?? date("Y")])}}"
